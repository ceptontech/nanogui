...
void Widget::removeGroup(Group *group) { mGroups.erase(group); }

void Widget::setTheme(Theme *theme) {
  if (mTheme.get() == theme) return;
  mTheme = theme;
...
void Widget::performLayout(NVGcontext *ctx) {
  if (mLayout) {
    ...
    for (auto c : mChildren) {
      ...
      c->setSize(
          Vector2i(fix[0] ? fix[0] : pref[0], fix[1] ? fix[1] : pref[1]));
      c->performLayout(ctx);
    }
  }
}
...
bool Widget::mouseButtonEvent(const Vector2i &p, int button, bool down,
                              int modifiers) {
  for (auto it = mChildren.rbegin(); it != mChildren.rend(); ++it) {
    Widget *child = *it;
    if (child->visible() && child->contains(p - mPos) &&
...
bool Widget::keyboardEvent(int, int, int, int) { return false; }

bool Widget::keyboardCharacterEvent(unsigned int) { return false; }
...
bool Widget::hasChild(const Widget *widget) const {
  return (std::find(mChildren.begin(), mChildren.end(), widget) !=
          mChildren.end());
}
...
void Widget::addChild(Widget *widget) { addChild(childCount(), widget); }

void Widget::removeChild(Widget *widget) {
  if (!hasChild(widget)) return;
  mChildren.erase(std::remove(mChildren.begin(), mChildren.end(), widget),
                  mChildren.end());
